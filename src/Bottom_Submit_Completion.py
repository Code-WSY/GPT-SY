...
# 检查格式并发送
def check_and_sendGPT_Completion(text, temperature, max_token, selected_model, selected_prompt_title,
                      selected_mode
                      ):
...
def sumbit_text_Completion(event):
    # -------------------------------提交用户输入参数----------------------------------------#
    text, temperature, max_token, selected_model, selected_prompt,\
        selected_mode= submit_user_parameter()
    # -------------------------------------------------------------------------------------#
    # -------------------------------检查格式并发送------------------------------------------#
    check_and_sendGPT_Completion(text, temperature, max_token, selected_model, selected_prompt, selected_mode)



#-----------------------------------------------------------------------------------#
# 提交按钮
submit_button_Completion = tk.Button(window,
                          text="提交",
                          width=submit_button_size[0],
                          height=submit_button_size[1],
                          command=lambda: sumbit_text_Completion(None),
                          )
# 按钮的字体
submit_button_Completion.config(
...
if __name__ == "__main__":
    submit_button_Completion.grid(row=1, column=0, sticky=tk.NSEW)
    window.mainloop()
